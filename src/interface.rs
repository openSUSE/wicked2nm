...
use agama_server::network::model::{
    self, DhcpSettings, IpConfig, IpRoute, Ipv4Method, Ipv6Method, MacAddress,
};
use anyhow::anyhow;
use cidr::IpInet;
...
pub struct Ipv6Dhcp {
    ...
    pub flags: String,
    // ignored
    #[serde(default = "default_v6_update")]
    pub update: String,
    // ignored
...
impl Interface {
    ...
    pub fn to_ip_config(&self) -> Result<IpConfigResult, anyhow::Error> {
        ...
        if let Some(ipv4_static) = &self.ipv4_static {
            if let Some(addresses_in) = &ipv4_static.addresses {
                for addr in addresses_in {
                    addresses.push(IpInet::from_str(addr.local.as_str())?);
                }
            }
        ...
        let mut dhcp4_settings: Option<DhcpSettings> = None;
        if let Some(ipv4_dhcp) = &self.ipv4_dhcp {
            let mut dhcp_settings = DhcpSettings::default();
            ...
            dhcp_settings.send_release = ipv4_dhcp.release_lease;
            dhcp4_settings = Some(dhcp_settings);
        }

        let mut dhcp6_settings: Option<DhcpSettings> = None;
        if let Some(ipv6_dhcp) = &self.ipv6_dhcp {
            ...
            dhcp_settings.send_release = ipv6_dhcp.release_lease;
...
macro_rules! check_unhandled_field {
    ($field:expr, $default:expr, $warnings:expr, $interface:expr) => {
        if $field != $default {
...
fn check_ignored(interface: &Interface) -> Vec<anyhow::Error> {
    ...
    check_unhandled_field!(ipv4.arp_verify, ipv4_default.arp_verify, warnings, interface);

    let ipv6 = &interface.ipv6;
    let ipv6_default = Ipv6::default();
    check_unhandled_field!(ipv6.accept_redirects, ipv6_default.accept_redirects, warnings, interface);

    if let Some(ipv4_dhcp) = &interface.ipv4_dhcp {
        let ipv4_dhcp_default = Ipv4Dhcp::default();
        check_unhandled_field!(ipv4_dhcp.flags, ipv4_dhcp_default.flags, warnings, interface);
        check_unhandled_field!(ipv4_dhcp.update, ipv4_dhcp_default.update, warnings, interface);
        check_unhandled_field!(
            ipv4_dhcp.defer_timeout,
            ipv4_dhcp_default.defer_timeout,
            warnings,
            interface
        );
        check_unhandled_field!(ipv4_dhcp.recover_lease, ipv4_dhcp_default.recover_lease, warnings, interface);
        check_unhandled_field!(ipv4_dhcp.release_lease, ipv4_dhcp_default.release_lease, warnings, interface);
    }

    if let Some(ipv6_dhcp) = &interface.ipv6_dhcp {
        let ipv6_dhcp_default = Ipv6Dhcp::default();
        check_unhandled_field!(ipv6_dhcp.flags, ipv6_dhcp_default.flags, warnings, interface);

        check_unhandled_field!(ipv6_dhcp.update, ipv6_dhcp_default.update, warnings, interface);
        check_unhandled_field!(ipv6_dhcp.rapid_commit, ipv6_dhcp_default.rapid_commit, warnings, interface);
        check_unhandled_field!(
            ipv6_dhcp.defer_timeout,
            ipv6_dhcp_default.defer_timeout,
            warnings,
            interface
        );
        check_unhandled_field!(ipv6_dhcp.recover_lease, ipv6_dhcp_default.recover_lease, warnings, interface);
        check_unhandled_field!(ipv6_dhcp.refresh_lease, ipv6_dhcp_default.refresh_lease, warnings, interface);
        check_unhandled_field!(ipv6_dhcp.release_lease, ipv6_dhcp_default.release_lease, warnings, interface);
    }

    warnings
}

// fn is_default<T: PartialEq + Default + fmt::Debug>(t: &T) -> bool {
...
mod tests {
    ...
    fn test_dhcp_interface_to_connection() {
        setup_default_migration_settings();
        let static_interface = Interface {
            ipv4_dhcp: Some(Ipv4Dhcp { enabled: true, ..Default::default() }),
            ..Default::default()
        };
    ...
    fn test_ignored_default() {
        ...
        println!("{:?}", check_ignored(&ifc));
        assert!(check_ignored(&ifc).len() == 12);
    }
}
